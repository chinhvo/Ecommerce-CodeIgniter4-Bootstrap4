...
function getPositionRelativeToParent(element, parentId) {
    ...
    return {
        ...
        bottom: parentRect.bottom - elementRect.bottom, // Distance from parent's bottom
        right: parentRect.right - elementRect.right    // Distance from parent's right
    };
}
...
$('.editCategorieSub').click(function () {
    ...
    var subFor = $(this).data('sub-for');
    $('[name="newSubIs"]').val(subFor);
...
function forceCloseEditor(event) {
	setTimeout(() => {
	  if (!$(event.target).closest('#categorieSubEdit').length && !$(event.target).closest('.editCategorieSub').length  && $('#categorieSubEdit').is(":visible")) {
	   	$('#categorieSubEdit').hide();
      }
  	}, 300);
...
function clearInputs(elementId) {
    ...
    inputs.forEach(input => {
        if (!myInput.readOnly && (input.type === 'checkbox' || input.type === 'radio')) {
            input.checked = false;
        } else {
            input.value = '';
        }
