...
// https://github.com/Studio-42/elFinder/wiki/Connector-configuration-options .'\attachments\uploads\'

$user_upload_folder_path = preg_replace('/assets.*/', '', realpath(dirname(__FILE__))) . 'attachments/uploads';
$upload_folder_url_path = preg_replace('/assets.*/', '', dirname($_SERVER['PHP_SELF'])) . 'attachments/uploads';

$user_upload_trash_folder_path = $user_upload_folder_path . '/.tmp';
$upload_trash_folder_url_path = $upload_folder_url_path . '/attachments/uploads/.tmp';

if (! is_dir($user_upload_folder_path)) {
    mkdir($user_upload_folder_path, 0777, true);
}
if (! is_dir($$user_upload_trash_folder_path)) {
    mkdir($user_upload_trash_folder_path, 0777, true);
}
$opts = array(
    // 'debug' => true,
    'roots' => array(
        ...
        array(
            ...
            'path' => $user_upload_folder_path, // path to files (REQUIRED)
            'URL' => $upload_folder_url_path, // URL to files (REQUIRED)
        ...
        array(
            'id' => '1',
            'driver' => 'Trash',
            'path' => $user_upload_trash_folder_path,
            'tmbURL' => $upload_trash_folder_url_path,
            'winHashFix' => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
            'uploadDeny' => array(
                'all'
            ), // Recomend the same settings as the original volume that uses the trash
            'uploadAllow' => array(
                'image',
                'text/plain'
            ), // Same as above
