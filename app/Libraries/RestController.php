...
use Config\Services;
use Config\Rest; // Your custom config class for REST settings
/**
...
abstract class RestController extends Controller {
    ...
    {
        helper(['url']); // Instead of loading URL helper in CI3

        $this->preflight_checks();

        // Load config (instead of $this->config->item)
        $restConfig = config(Rest::class);

        // Global XSS filtering (if needed — CI4 does it differently)
        $this->_enable_xss = $restConfig->globalXssFiltering ?? false;
        ...
        // Supported formats
        $supported_formats = $restConfig->supportedFormats ?? [];
        ...
        $this->supportedFormats = array_intersect_key($this->_supported_formats, array_flip($supported_formats));

        // Load language
        $language = $restConfig->language ?? 'english';
        ...
        // Check CORS
        if ($restConfig->checkCors ?? false) {
            $this->_check_cors();
        }

        // Merge GET variables
        $this->_get_args = array_merge(
            $this->request->getGet(),
            $this->request->getUri()->getSegments()
        );

        // Detect input format
        $this->requestData['format'] = $this->_detect_input_format();
        $this->requestData['body']   = $this->request->getBody();

        if ($this->requestData['format'] && $this->requestData['body']) {
            $this->requestData['body'] = $this->format
                ->factory($this->requestData['body'], $this->requestData['format'])
                ->to_array();
        }

        // Headers
        $this->_head_args = $this->request->getHeaders();
        ...
        $this->response->setHeader('Content-Type', $this->_detect_output_format());

        // Detect language
        $this->response->setHeader('Content-Language', $this->_detect_lang());

        // Early checks
        $this->early_checks();

        // Load database if enabled
        if ($restConfig->databaseGroup && ($restConfig->enableKeys || $restConfig->enableLogging)) {
            $this->restDb = \Config\Database::connect($restConfig->databaseGroup);
        }
        ...
        // API key check
        if ($restConfig->enableKeys && $this->auth_override !== true) {
            $this->_allow = $this->_detect_api_key();
        }

        // AJAX only check
        if (!$this->request->isAJAX() && ($restConfig->ajaxOnly ?? false)) {
            return $this->response->setJSON([
                $restConfig->statusFieldName  => false,
                $restConfig->messageFieldName => lang('Rest.text_rest_ajax_only')
            ])->setStatusCode(406);
        }

        // Default auth
        if ($this->auth_override === false &&
            (!($restConfig->enableKeys && $this->_allow === true) ||
                (($restConfig->allowAuthAndKeys ?? false) && $this->_allow === true))
        ) {
            $restAuth = strtolower($restConfig->restAuth);
            switch ($restAuth) {
                case 'basic':
            ...
            if ($restConfig->ipWhitelistEnabled ?? false) {
