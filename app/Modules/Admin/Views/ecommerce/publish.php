...
<form method="POST" action="<?= base_url() ?>admin/publish" enctype="multipart/form-data">
	...
	foreach ($languages as $language) {
		...
		<div class="locale-container locale-container-<?= htmlspecialchars($language->abbr) ?>" <?= $language->abbr == MY_DEFAULT_LANGUAGE_ABBR ? 'style="display:block;"' : '' ?>>
			...
			<div class="form-group">
				<label for="title<?= $i ?>"><?= lang('title') ?> (<?= htmlspecialchars($language->name) ?>
					<img src="<?= base_url('attachments/lang_flags/' . $language->flag) ?>" alt="">)
				</label> 
			...
			<div class="form-group">
				<a href="javascript:void(0);" class="btn btn-secondary showSliderDescrption" data-descr="<?= $i ?>">
					<?= lang('slider_description') ?> <i class="fa fa-arrow-circle-down"></i>
				</a>
			</div>
			<div class="theSliderDescrption" id="theSliderDescrption-<?= $i ?>" <?= isset($_POST['in_slider']) && $_POST['in_slider'] == 1 ? 'style="display:block;"' : '' ?>>
				<div class="form-group">
					<label for="basic_description<?= $i ?>"><?= lang('slider_description') ?> (<?= htmlspecialchars($language->name) ?><img src="<?= base_url('attachments/lang_flags/' . $language->flag) ?>" alt="">)
					</label>
			...
			<div class="form-group">
				<label for="description<?= $i ?>"><?= lang('description') ?> (<?= htmlspecialchars($language->name) ?><img src="<?= base_url('attachments/lang_flags/' . $language->flag) ?>" alt="">)
				</label>
			...
			<div class="form-group for-shop">
				<label for="price<?= $i ?>"><?= lang('price') ?> (<?= htmlspecialchars($language->name) ?><img src="<?= base_url('attachments/lang_flags/' . $language->flag) ?>" alt="">)
				</label> 
				<input type="text" id="price<?= $i ?>" name="price[]" placeholder="<?= lang('without_currency_at_end') ?>"
					value="<?= $trans_load != null && isset($trans_load[$language->abbr]['price']) ? $trans_load[$language->abbr]['price'] : '' ?>" class="form-control">
			</div>
			<div class="form-group for-shop">
				<label for="old_price<?= $i ?>"><?= lang('old_price') ?> (<?= htmlspecialchars($language->name) ?><img src="<?= base_url('attachments/lang_flags/' . $language->flag) ?>" alt="">)
				</label> 
				<input type="text" id="old_price<?= $i ?>" name="old_price[]" placeholder="<?= lang('without_currency_at_end') ?>"
	...
	<div class="form-group bordered-group">
		...
		?>
		<label for="userfile"><?= lang('cover_image') ?></label> <input type="file" id="userfile" name="userfile">
	</div>
	<div class="form-group bordered-group">
		<div class="others-images-container">
			<?= $otherImgs ?>
		</div>
		<a href="javascript:void(0);" data-toggle="modal" data-target="#modalMoreImages" class="btn btn-secondary"><?= lang('upload_more_images') ?></a>
	</div>
	<div class="form-group for-shop">
		<label for="shop_categorie"><?= lang('shop_categories') ?></label> 
	...
	<div class="form-group for-shop">
		...
		<input type="number" placeholder="<?= lang('number') ?>" name="quantity"
	...
	if ($virtualProducts == 1) { ?>
		<div class="form-group for-shop">
			<label for="virtual_products"><?= lang('virtual_products') ?> 
	...
	<?php } ?>
	<div class="form-group for-shop">
			<label for="in_slider"><?= lang('in_slider') ?></label> 
		...
		<div class="form-group for-shop">
			...
			<select class="selectpicker" name="highlighted" id="highlighted">
				<option value="1" <?= isset($_POST['highlighted']) && $_POST['highlighted'] == 1 ? 'selected' : '' ?>><?= lang('yes') ?></option>
				<option value="0" <?= isset($_POST['highlighted']) && $_POST['highlighted'] == 0 || !isset($_POST['highlighted']) ? 'selected' : '' ?>><?= lang('no') ?></option>
			</select>
		</div>
		<div class="form-group for-shop">
			<label for="position"><?= lang('position') ?></label> 
			<input type="number" placeholder="<?= lang('position_number') ?>" name="position" id="position"
				value="<?= isset($_POST['position']) ? htmlspecialchars($_POST['position']) : '' ?>" class="form-control">
		</div>
	<button type="submit" name="submit" class="btn btn-lg btn-primary btn-publish" value="save"><?= lang('publish') ?></button>
	<?php if ($uri->getSegment(3) !== null) { ?>
		<a href="<?= base_url('admin/products') ?>" class="btn btn-lg btn-secondary"><?= lang('cancel') ?></a>
	<?php } ?>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="myModalLabel"><?= lang('upload_more_images') ?></h5>
				...
				<div class="modal-body">
					<form id="uploadImagesForm">
						...
						<label for="others"><?= lang('select_images') ?></label> 
	...
	<div class="modal fade" id="virtualProductsHelp" tabindex="-1" role="dialog" aria-labelledby="virtualProductsHelp" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title"><?= lang('what_are_virtual_products') ?></h5>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<?= lang('virtual_products_help_text') ?>
				</div>
				<div class="modal-footer">
