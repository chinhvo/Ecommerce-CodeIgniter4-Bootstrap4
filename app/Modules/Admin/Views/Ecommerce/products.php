	...
	<div class="container-fluid my-4" id="products">
        ...
        <h1>
			<img src="<?= base_url('assets/imgs/products-img.png') ?>"
				class="header-img" style="margin-top: -2px;"> Products
		</h1>
		<hr>
		<div class="row">
			<div
				...
				<form method="get" id="searchProductsForm"
					...
					<div class="form-row">
						...
						<div class="form-group col-sm-4">
							<label for="order_by">Order:</label> <select id="order_by"
								name="order_by" class="form-control change-products-form">
								<option value="id=desc"
									<?= (service('request')->getGet('order_by') === 'id=desc') ? 'selected' : '' ?>>Newest</option>
								<option value="id=asc"
									<?= (service('request')->getGet('order_by') === 'id=asc') ? 'selected' : '' ?>>Latest</option>
								<option value="quantity=asc"
									<?= (service('request')->getGet('order_by') === 'quantity=asc') ? 'selected' : '' ?>>Low
									Quantity</option>
								<option value="quantity=desc"
									<?= (service('request')->getGet('order_by') === 'quantity=desc') ? 'selected' : '' ?>>High
									Quantity</option>
							</select>
						</div>
						...
						<div class="form-group col-sm-4">
							...
							<div class="input-group">
								<input id="search_title" class="form-control"
									placeholder="Product Title" type="text" name="search_title"
		...
		<div class="table-responsive">
            <?php if (!empty($products)): ?>
				...
				<tbody>
                    ...
                    ?>
                            <tr>
						<td><img src="<?= $image ?>" alt="No Image" class="img-thumbnail"
						...
						<td>
                                    <?=($row->vendor_id > 0) ? '<a href="?show_vendor=' . esc($row->vendor_id) . '">' . esc($row->vendor_name) . '</a>' : 'No vendor'?>
                                </td>
						...
						<td class="text-right"><a
							...
							class="btn btn-danger btn-sm confirm-delete"> Delete </a></td>
					</tr>
                        <?php endforeach; ?>
                    </tbody>
			</table>
