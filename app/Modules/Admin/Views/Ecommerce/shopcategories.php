...
<div class="col-sm-9 col-md-9 col-lg-10 offset-sm-3 offset-md-3 offset-lg-2 pt-2">
	<div id="languages">
		...
		<?php if (isset($validation) && count($validation->getErrors()) > 0): ?>
			<div class="alert alert-danger alert-dismissible" role="alert">
				...
				<?= $this->include('\App\Views\close_alert') ?>
			</div>
			<hr>
		<?php endif; ?>
				...
				<table class="table table-striped custab">
					...
					?>
						<tr>
							...
							<td><?= $catName ?></td>
							<td>
								<a href="javascript:void(0);" class="editCategorieIcon"
									data-for-id="<?= $key_cat ?>"
									data-icon="<?= esc($shop_categorie['icon'] ?? '') ?>"
									data-toggle="tooltip" data-placement="top" title="<?= lang('edit') ?>">
									<i class="fa fa-pencil"></i>
								</a>
								<?php if (! empty($shop_categorie['icon'])): ?>
									<i id="icon-preview-<?= $key_cat ?>" class="<?= esc($shop_categorie['icon']) ?>" aria-hidden="true"></i>
									<small id="icon-value-<?= $key_cat ?>" class="text-muted d-block"><?= esc($shop_categorie['icon']) ?></small>
									<small id="icon-empty-<?= $key_cat ?>" class="text-muted d-none">-</small>
								<?php else: ?>
									<i id="icon-preview-<?= $key_cat ?>" class="d-none" aria-hidden="true"></i>
									<small id="icon-value-<?= $key_cat ?>" class="text-muted d-block"></small>
									<small id="icon-empty-<?= $key_cat ?>" class="text-muted">-</small>
								<?php endif; ?>
							</td>
							<td>
	...
	<div id="categorieSubEdit">
		<form method="POST" id="categorieEditSubChanger" action="<?= base_url('admin/shopcategories') ?>">
