...
<div class="row">
    <div class="col-md-6 col-md-offset-3">
        <?php
        if ($this->session->getFlashdata('result_publish')) {
            ?> 
            <div class="alert alert-success"><?= $this->session->getFlashdata('result_publish') ?></div> 
            <?php
        }
        ?>
        <div class="content">
            <form class="form-box" action="" method="POST" enctype="multipart/form-data">
                ...
                foreach ($languages as $language) {
                    ...
                    <div class="locale-container locale-container-<?= htmlspecialchars($language->abbr) ?>" <?= $language->abbr == MY_DEFAULT_LANGUAGE_ABBR ? 'style="display:block;"' : '' ?>>
                        ...
                        <label for="description<?= $i ?>"><?= lang('vendor_product_description') ?> <img src="<?= base_url('attachments/lang_flags/' . htmlspecialchars($language->flag)) ?>" alt="<?= htmlspecialchars($language->name) ?>"></label>
                        <div class="form-group">
                ...
                <div class="form-group bordered-group">
                    ...
                    ?>
                    <label for="vendor_userfile"><?= lang('vendor_cover_image') ?></label>
                    <input type="file" name="userfile" id="vendor_userfile">
                </div>
                <div class="form-group bordered-group">
                ...
                <div class="form-group">
                    <label for="vendor_shop_categorie"><?= lang('vendor_select_category') ?></label>
                    <select class="selectpicker form-control show-tick show-menu-arrow" name="shop_categorie" id="vendor_shop_categorie">
                ...
                <?php if ($showBrands == 1) { ?>
                    <div class="form-group for-shop">
                        <label for="vendor_brand_id"><?= lang('brand') ?></label>
                        <select class="selectpicker" name="brand_id" id="vendor_brand_id">
