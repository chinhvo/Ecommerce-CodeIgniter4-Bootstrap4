...
})(window,document,'script','dataLayer','GTM-KM3HH9B4');</script>
...
<title><?= $title ?></title>
<link rel="icon" href="assets/imgs/favicon.jpg" type="image/jpg">
<link rel="stylesheet"
...
<body>
	...
	<noscript>
		<iframe src="https://www.googletagmanager.com/ns.html?id=GTM-KM3HH9B4"
			height="0" width="0" style="display: none; visibility: hidden"></iframe>
	</noscript>
	<!-- End Google Tag Manager (noscript) -->
	<a style="display: none !important;" id="kk-refer-gh"
		href="https://github.com/kirilkirkov">Kiril Kirkov</a>
	<div id="wrapper">
		<div id="content">
            <?php if ($multiVendor == 1) { ?>
				...
				<div class="container">
					<a href="<?= LANG_URL . '/vendor/register' ?>"
						class="btn btn-default"><?= lang('register_me') ?></a>
				...
				<div class="container">
                        ...
                        if ($num_langs > 0) {
                            ...
                            foreach ($allLanguages as $key_lang => $lang) {
                                ?>
                                    <li
							<?= $i == $num_langs ? 'class="last-item"' : '' ?>><img
                        ...
                        <div class="phone pull-right">
                            ...
                            if ($footerContactPhone != '') {
                                ?>
                                <img
							src="<?= base_url('template/imgs/Phone-icon.png') ?>"
							alt="Call us"> <a href="tel:<?= $footerContactPhone?>"
							class="nav-top-link" target="_blank"><?php echo $footerContactPhone ?></a>
                                <?php
                            }
                            ?>
                        </div>
			...
			<div id="top-part">
				<div class="container">
					<div class="row header-if">
						<div class="col-sm-12 col-md-3 col-lg-3 left site-if">
							<a href="<?= base_url() ?>"> <img
								src="<?= base_url('attachments/site_logo/' . $sitelogo) ?>"
								class="site-logo" alt="<?= $_SERVER['HTTP_HOST'] ?>">
							</a>
							<h4 class="site-name">
								<a href="<?= base_url() ?>">Tâm Nông Garden</a>
							</h4>
						</div>
						<div class="col-sm-6 col-md-5 col-lg-6">
							<div class="input-group" id="adv-search">
								<input type="text"
									value="<?= isset($_POST['search_in_title']) ? htmlspecialchars($_POST['search_in_title']) : '' ?>"
									id="search_in_title" class="form-control search-field-header"
								...
								<div class="input-group-btn">
									<div class="btn-group" role="group">
										<div class="dropdown dropdown-lg">
											<div class="dropdown-menu dropdown-menu-right" role="menu">
												<form class="form-horizontal" method="POST"
													action="<?= isset($vendor_url) ? $vendor_url : LANG_URL ?>"
													id="bigger-search">
													<input type="hidden" name="category"
														value="<?= isset($_POST['category']) ? htmlspecialchars($_POST['category']) : '' ?>">
													<input type="hidden" name="in_stock"
														value="<?= isset($_POST['in_stock']) ? htmlspecialchars($_POST['in_stock']) : '' ?>">
													<input type="hidden" name="search_in_title"
														value="<?= isset($_POST['search_in_title']) ? htmlspecialchars($_POST['search_in_title']) : '' ?>">
													<input type="hidden" name="order_new"
														value="<?= isset($_POST['order_new']) ? htmlspecialchars($_POST['order_new']) : '' ?>">
													<input type="hidden" name="order_price"
														value="<?= isset($_POST['order_price']) ? htmlspecialchars($_POST['order_price']) : '' ?>">
													<input type="hidden" name="order_procurement"
														value="<?= isset($_POST['order_procurement']) ? htmlspecialchars($_POST['order_procurement']) : '' ?>">
													<input type="hidden" name="brand_id"
														value="<?= isset($_POST['brand_id']) ? htmlspecialchars($_POST['brand_id']) : '' ?>">
													<div class="form-group">
														<label for="quantity_more"><?= lang('quantity_more_than') ?></label>
														<input type="number"
															value="<?= isset($_POST['quantity_more']) ? htmlspecialchars($_POST['quantity_more']) : '' ?>"
													...
													<div class="row">
														<div class="col-md-6">
															<div class="form-group">
																...
																<div class="input-group date">
																	<input type="text"
																		value="<?= isset($_POST['added_after']) ? htmlspecialchars($_POST['added_after']) : '' ?>"
														...
														<div class="col-md-6">
															<div class="form-group">
																...
																<div class="input-group date">
																	<input type="text"
																		value="<?= isset($_POST['added_before']) ? htmlspecialchars($_POST['added_before']) : '' ?>"
													...
													<div class="form-group">
														...
														<input class="form-control"
															value="<?= isset($_POST['search_in_body']) ? htmlspecialchars($_POST['search_in_body']) : '' ?>"
													...
													<div class="row">
														<div class="col-md-6">
															<div class="form-group">
																...
																<input type="text"
																	value="<?= isset($_POST['price_from']) ? htmlspecialchars($_POST['price_from']) : '' ?>"
														...
														<div class="col-md-6">
															<div class="form-group">
																<label for="price_to"><?= lang('price_to') ?></label> <input
																	...
																	value="<?= isset($_POST['price_to']) ? htmlspecialchars($_POST['price_to']) : '' ?>"
										...
										<button type="button" onclick="submitForm()"
											...
											<img src="<?= base_url('template/imgs/search-ico.png') ?>"
												alt="Search">
										</button>
									</div>
								</div>
									...
									<tr>
										<td>
											<ul class="shop-dropdown">
												<li class="dropdown text-center"><a href="#"
													class="dropdown-toggle" data-toggle="dropdown"
													role="button" aria-expanded="false">
														<div>
															<span class="sumOfItems"><?= is_numeric($cartItems) && (int)$cartItems == 0 ? 0 : $sumOfItems ?></span> <?= lang('items') ?></div>
														<img
														src="<?= base_url('template/imgs/shopping-cart-icon-515.png') ?>"
										...
										<td>
											<div class="center">
												...
												<span class="vertical-divider"></span> <a
													href="<?= LANG_URL . '/shopping-cart' ?>"><?= lang('shopping_cart_only') ?></a>
											</div>
										</td>
									</tr>
								</table>
						...
						<div class="col-sm-6 col-md-4 col-lg-2">
							<ul class="header-nav topbar_left">
								<li class="zalo"><a
									href="http://zalo.me/<?= $footerContactPhone?>"
									class="nav-top-link" target="_blank">Zalo: 
									<?php echo $footerContactPhone ?>
									</a></li>
							</ul>
						</div>
					</div>
				...
				<div class="container">
					<div class="navbar-header">
						<button type="button" class="navbar-toggle collapsed"
							...
							href="<?= base_url() ?>"><?= $navitext ?></a>
                            <?php } ?>
                        </div>
					<div id="navbar" class="collapse navbar-collapse">
						<ul class="nav navbar-nav" style="<?= $navitext == null ? 'margin-left:-15px;' : '' ?>">
							<li
								<?= uri_string() == '' || uri_string() == MY_LANGUAGE_ABBR ? ' class="active"' : '' ?>>
								<a href="<?= LANG_URL ?>"> <i class="fa fa-home"></i>
										<?= lang('home') ?>
									</a>
							</li>
                                <?php
                                if (! empty($nonDynPages)) {
                                    foreach ($nonDynPages as $addonPage) {
                                        ?>
                                        <li
								<?= uri_string() == $addonPage || uri_string() == MY_LANGUAGE_ABBR . '/' . $addonPage ? ' class="active"' : '' ?>><a
								href="<?= LANG_URL . '/' . $addonPage ?>"><?= mb_ucfirst(lang($addonPage)) ?></a></li>
                                        <?php
                                    }
                                }
                                if (! empty($dynPages)) {
                                    foreach ($dynPages as $addonPage) {
                                        ?>
                                        <li
								<?=urldecode(uri_string()) == 'page/' . $addonPage['pname'] || uri_string() == MY_LANGUAGE_ABBR . '/' . 'page/' . $addonPage['pname'] ? ' class="active"' : ''?>><a
								href="<?= LANG_URL . '/page/' . $addonPage['pname'] ?>"><?= mb_ucfirst($addonPage['lname']) ?></a></li>
                                            <?php
                                    }
                                }
                                ?>
                              <li
								<?= uri_string() == 'contacts' || uri_string() == MY_LANGUAGE_ABBR . '/contacts' ? ' class="active"' : '' ?>><a
								href="<?= LANG_URL . '/contacts' ?>"><?= lang('contacts') ?></a></li>
						</ul>
					</div>
				</div>
			</nav>