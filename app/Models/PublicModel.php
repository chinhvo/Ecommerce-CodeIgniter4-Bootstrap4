...
class PublicModel extends Model
{
    ...
    public function __construct()
    {
        ...
        helper(['text', 'url', 'vntostr', 'except_letters']);
    ...
    public function productsCount(array $big_get = [])
    {
        ...
        if (!empty($big_get)) {
            $this->getFilter($big_get, $builder);
        }
    ...
    public function getNewProducts()
    {
        ...
        $limit = (int) $this->HomeAdminModel->getValueStore('newProductsLimit');
        $builder->limit($limit > 0 ? $limit : 5);
    ...
    public function getLastBlogs()
    {
        ...
        $limit = (int) $this->HomeAdminModel->getValueStore('lastBlogsLimit');
        $builder->limit($limit > 0 ? $limit : 6);
    ...
    private function getFilter(array $big_get, $builder = null)
    {
        $builder = $builder ?? $this->db->table('products');

        if (!empty($big_get['category'])) {
            $categoryId = $this->resolveCategoryId($big_get['category']);

            if ($categoryId === null) {
                $builder->where('products.shop_categorie', 0);

                return $builder;
            }

            $findInIds = [$categoryId];
        ...
        if (!empty($big_get['search_in_title'])) {
            $builder->groupStart()
                ->like('products_translations.title', $big_get['search_in_title'])
                ->orLike('products_translations.description', $big_get['search_in_title'])
                ->groupEnd();
        }
    ...
    public function getShopCategories()
    {
        ...
        $builder->select('shop_categories.sub_for, shop_categories.id, shop_categories.icon, shop_categories_translations.name, shop_categories_translations.url');
        ...
        if ($query->getNumRows() > 0) {
            foreach ($query->getResultArray() as $row) {
                $row['slug'] = !empty($row['url'])
                    ? $row['url']
                    : $this->buildCategorySlug($row['name'], (int) $row['id']);
                $arr[] = $row;
            }
        }
    ...
    private function resolveCategoryId($category): ?int
    {
        ...
        if (preg_match('/-(\d+)$/', $categorySlug, $matches) === 1) {
            return (int) $matches[1];
        }

        foreach ($this->getShopCategories() as $shopCategory) {
            if (($shopCategory['slug'] ?? '') === $categorySlug) {
                return (int) $shopCategory['id'];
            }
        }
    ...
    public function getHighlightedProducts(array $big_get = [])
    {
        $builder = $this->db->table('products');
        $builder->select('vendors.url as vendor_url, products.id, products.quantity, products.image, products.url, products_translations.price, products_translations.title, products_translations.basic_description, products_translations.old_price');
        $builder->join('products_translations', 'products_translations.for_id = products.id', 'left');
